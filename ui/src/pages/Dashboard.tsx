    ...
    <div className="space-y-6">
        ...
        <div className="relative grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
          <div>
            ...
            <h2 className="max-w-xl text-3xl font-semibold leading-snug tracking-tight text-foreground">
              エージェントの稼働状況を<br />ひとつの管制室で。
            </h2>
            <p className="mt-4 max-w-lg text-[14px] leading-relaxed text-muted-foreground">
              タスク進行、承認フロー、コスト推移をリアルタイムで可視化。チームのAIエージェントを一元管理します。
            </p>
          </div>
          ...
          <div className="glass-subtle rounded-[20px] p-5">
            ...
            <div className="space-y-2">
              {query.data.activities.slice(0, 3).map((act) => (
                <div key={act.id} className="rounded-[12px] border border-[var(--glass-border)] bg-[var(--glass-bg)] p-3 backdrop-blur-sm transition-colors hover:bg-[var(--glass-bg-strong)]">
                  <p className="text-[13px] font-medium text-foreground">{act.title}</p>
                  <p className="mt-0.5 text-[11px] text-muted-foreground">{formatDate(act.occurredAt)}</p>
                </div>
              ))}
              {query.data.activities.length === 0 && (
                <p className="py-4 text-center text-[12px] text-muted-foreground">まだアクティビティがありません</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <div className="glass rounded-[24px] p-6">
          ...
          <div className="space-y-2">
            {query.data.activities.map((act) => (
              <div key={act.id} className="flex items-start justify-between gap-3 rounded-[14px] border border-[var(--glass-border)] bg-[var(--glass-bg-subtle)] p-4 transition-colors hover:bg-[var(--glass-bg)]">
                <div className="min-w-0">
                  <p className="text-[13px] font-medium text-foreground">{act.title}</p>
                  <p className="mt-1 text-[12px] text-muted-foreground">{act.description}</p>
                </div>
                <span className="flex-shrink-0 text-[11px] text-muted-foreground">{formatDate(act.occurredAt)}</span>
              </div>
            ))}
            {query.data.activities.length === 0 && (
